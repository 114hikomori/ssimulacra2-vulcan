...
- [ ] Benchmark shared-original batch vs independent pairs: **requires the prep-cache implementation** (item above). Interim estimate from the 4K `--profile` stage split: original-side work ≈ **398 ms of the 1473 ms wall (~27%)** — but note this is a CONSERVATIVE floor: it sums record-side stage times, so original-side kernels' actual GPU time (hidden inside the per-scale submit-wait, 122 ms) is not credited; realistic ceiling after implementation likely 30-40% of per-image wall at 4K. Treat 27% as the minimum worth implementing for; full A/B after the refactor.
